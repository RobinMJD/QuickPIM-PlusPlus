...
The saved notification preference is now reconciled with the optional permission granted to each browser installation. Settings clearly reports a missing permission, provides a user-initiated repair and test action, and includes the effective state in sanitized diagnostics. Permission changes immediately reschedule request tracking. Expiry delivery now catches up for recently missed alarms after browser sleep and backs off failed delivery attempts to avoid a tight retry loop.
